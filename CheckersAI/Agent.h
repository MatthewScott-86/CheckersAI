...
public:
	...
	MoveSequence MiniMax()
	{
		if (m_board.AttackOnBoard(m_color))
			return MiniMaxAttacks();
		else
			return MiniMaxMoves();
	}

	MoveSequence MiniMaxMoves()
	{
		...
		{
			auto start = piece.first;
			for (auto move : piece.second->getMoves())
			{
				auto end = Common::PositionPlusMove(start, move);
				if (!m_board.ValidMove(m_color, end))
					continue;
				...
				auto moveTest = tempBoard.Move(start, end, m_color);
				assert(moveTest);
				tempMoves.push_back({ start, end });
				tempValue = MinValue(tempBoard, Common::OtherColor(m_color), INT_MIN, INT_MAX, 1);
				
				MiniMaxDecision(value, tempValue, returnMoves, tempMoves, first);
			}
		}
		return returnMoves;
	}
	MoveSequence MiniMaxAttacks()
	{
		double value = INT_MIN;
		MoveSequence returnMoves;
		auto myPieces = m_board.getPieces(m_color);
		...
		for (auto piece : myPieces)
		{
			...
			{
				auto end = Common::PositionPlusMove(start, move);
				if (!m_board.ValidAttack(m_color, start, end))
					continue;
	...
	void MiniMaxDecision(double& value, double& testValue, MoveSequence& moves, MoveSequence& testMoves, bool& first)
	{
		if (testValue > value || first) {
			value = testValue;
			moves = testMoves;
			first = false;
		}
			...
			if (testValue + FLOAT_EPSILON > value && randNum > 0.5) {
				value = testValue;
				moves = testMoves;
			}
		}
